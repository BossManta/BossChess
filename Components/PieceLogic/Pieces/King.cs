...
public class King: Piece
{
    public override List<IMove> GetRawMoves(IBoard currentBoard, Point pos)
    {
        ...
        if (!king.HasMoved)
        {
            ...
            {
                if (IsPosEmptyAndSafe(currentBoard, new Point(pos.X,pos.Y), king.IsWhite) && IsPosEmptyAndSafe(currentBoard, new Point(pos.X+1,pos.Y), king.IsWhite) && IsPosEmptyAndSafe(currentBoard, new Point(pos.X+2,pos.Y) , king.IsWhite))
                {
            ...
            if (rRook.Type==PieceType.Rook && !rRook.HasMoved)
            {
                if (IsPosEmptyAndSafe(currentBoard, new Point(pos.X,pos.Y), king.IsWhite) && IsPosEmptyAndSafe(currentBoard, new Point(pos.X-1,pos.Y), king.IsWhite) && IsPosEmptyAndSafe(currentBoard, new Point(pos.X-2,pos.Y) , king.IsWhite) && IsPosEmptyAndSafe(currentBoard, new Point(pos.X-3,pos.Y) , king.IsWhite))
                {
                    IMove m = MoveFactory.MakeMove(pos, new Point(pos.X-2,pos.Y));
